...
import { Button, Card, Form, Input, InputNumber, message, Space, Table } from 'ant-design-vue';
...
const feeItems = ref<FeeRow[]>([{ key: `${Date.now()}`, name: '海运费', amount: 0 }]);
...
function addItem() {
  feeItems.value.push({ key: `${Date.now()}-${Math.random()}`, name: '', amount: 0 });
}
...
async function handleSaveQuote() {
  ...
  try {
    const quoteId = await createFreightQuote({
      ...
      quantity: 1,
      surcharge: 0,
      remark: JSON.stringify({ feeItems: feeItems.value, remark: form.remark }),
    });
...
<template>
  <Page auto-content-height title="报价编辑器">
    <template #extra>
      <Button @click="router.push('/tools/fba-flex')"> 返回工具中心 </Button>
    </template>
    ...
    <Card title="基础信息">
      <Form layout="inline">
        ...
        <Form.Item label="备注">
          <Input v-model:value="form.remark" placeholder="可选" style="width: 360px" />
        </Form.Item>
      </Form>
    </Card>
    ...
    <Card class="mt-4" title="费用项">
      ...
      <Table :data-source="feeItems" :columns="columns" :pagination="false" row-key="key">
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'name'">
            <Input v-model:value="record.name" placeholder="例如：报关费" />
          </template>
          <template v-else-if="column.key === 'amount'">
            <InputNumber v-model:value="record.amount" :min="0" :precision="2" style="width: 100%" />
          </template>
          <template v-else-if="column.key === 'actions'">
            <Button danger type="link" @click="removeItem(record.key)"> 删除 </Button>
          </template>
        </template>
      </Table>
      ...
      <div class="mt-4 flex justify-end">
        <Button type="primary" :loading="saving" @click="handleSaveQuote"> 保存并预览 </Button>
      </div>
    </Card>
  </Page>
</template>
