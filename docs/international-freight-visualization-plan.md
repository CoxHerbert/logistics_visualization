...
- 核心能力（当前阶段）：价格查看 + 订单状态查询；
- 状态维护方式：由内部人员在后台手动更新节点状态；
- 转化策略：门户负责展示与咨询，下单仍由销售/客服人工承接。
...
- 状态更新及时性：当天订单是否有最新状态；
- 状态准确性：抽检时状态与实际业务是否一致；
- 门户可用性：客户能否顺利查到价格与订单状态。

说明：现阶段不追求复杂经营指标，先把“能查、准、及时”做好。
...
暂不引入复杂能力（自动轨迹、复杂 KPI、多流程协同），避免前期投入过大。

这样可以先快速上线一个可用版本，后续再按业务增长逐步补齐自动化能力。

如果你愿意，我下一步可以直接给你：

- 一份最小可用的数据表（订单 + 状态日志 + 价格表）；
- 对应最小接口清单（后台录入、门户查询）；
- 最小页面结构（后台状态维护页 + 门户查询页）。
