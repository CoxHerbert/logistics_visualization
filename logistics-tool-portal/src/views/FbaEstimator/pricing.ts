...
// 特货附加：简单按 CBM 加价（示例）
function specialSurchargePerCbm(flags?: {
    isBattery?: boolean;
    isLiquid?: boolean;
    isPowder?: boolean;
    isMagnetic?: boolean;
...
export function estimate(
    ...
    if (serviceType === 'LCL') {
        const r = RATE.LCL;
        const min =
            cbm * (r.cnyPerCbmMin + addPerCbm) +
            r.originFixed +
            r.docFixed +
            cbm * r.destPerCbmMin +
            cbm * r.deliveryPerCbmMin;
        const max =
            cbm * (r.cnyPerCbmMax + addPerCbm) +
            r.originFixed +
            r.docFixed +
            cbm * r.destPerCbmMax +
            cbm * r.deliveryPerCbmMax;
